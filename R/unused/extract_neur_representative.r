...
extract_neur_representative <- function(SOM, filename, pdb, traj, neuron, append=FALSE){
    ...
    #Check that the neuron is a number
    if(inherits(neuron, "numeric")==FALSE){
        stop("neuron must be a number")
    }
    #Check that the number of frames of the SOM and of the trajectory are the same
    if(dim(SOM$data[[1]])[1] != dim(traj$coord)[3]){
        stop(paste("Number of frames used to train the SOM (", SOM$data[[1]])[1], ") and traj number of frames (", dim(traj$coord)[3],") are not consistent")
    }
    ...
    C <- traj$coords[,,seq(1, dim(traj$coords)[3])][,,REPRESENTATIVE]*10
    pdb$atom$x <- C[,1]
    pdb$atom$y <- C[,2]
    pdb$atom$z <- C[,3]
    pdb$xyz <- matrix(rbind(C[,1], C[,2], C[,3]), nrow=1)
    pdb$call <- sys.call()
    return(pdb)
#   bio3d::write.pdb(pdb, file=filename, xyz=matrix(rbind(C[,1], C[,2], C[,3]), nrow=1))
}
