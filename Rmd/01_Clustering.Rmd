...
Load the SOMMD and kohonen packages:
...
```{r}
xtcfile <- system.file("data/01_FOXP1_dataset/WT-1-PROD.xtc", package="SOMMD")
grofile <- system.file("data/01_FOXP1_dataset/WT.gro", package="SOMMD")
```
...
```{r}
### THE PRINT COMMAND IS NOT ALREADY DONE
#print(trj)
```
...
SOM <- kohonen::som(trj2xyz(trj.fit, inds=ca.inds), 
                    grid = somgrid(8, 8, "hexagonal", neighbourhood.fct="gaussian", toroidal=TRUE), 
...
sele.dists <- native.cont(struct=gro, distance=1.1, atoms=cb.inds)
...
RMSD_file <- xtcfile <- system.file("data/01_FOXP1_dataset/RMSD_WT-1-PROD.dat", package="SOMMD")
...
folder <- system.file("data/01_FOXP1_dataset/", package="SOMMD")
xtcfiles <- list.files(folder, pattern = "WT-.-PROD.xtc")
grofile <- system.file("data/01_FOXP1_dataset/WT.gro", package="SOMMD")

MD1 <- read.trj(trjfile=paste(folder, "/", xtcfiles[1], sep=''), topfile=grofile)
MD2 <- read.trj(trjfile=paste(folder, "/", xtcfiles[2], sep=''), topfile=grofile)
MD3 <- read.trj(trjfile=paste(folder, "/", xtcfiles[3], sep=''), topfile=grofile)
MD4 <- read.trj(trjfile=paste(folder, "/", xtcfiles[4], sep=''), topfile=grofile)
MD5 <- read.trj(trjfile=paste(folder, "/", xtcfiles[5], sep=''), topfile=grofile)
```
