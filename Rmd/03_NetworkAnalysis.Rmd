...
At first we read the simulation files (the gro and the trajectory files).
...
#Read the xtc file 
xtcfile <- system.file("data/03_HIF_dataset/HIF_MetaD.xtc", package="SOMMD")
grofile <- system.file("data/03_HIF_dataset/ref.gro", package="SOMMD")
trj <- read.trj(trjfile=xtcfile, topfile=grofile)
```
...
```{r}
SOM_nat <- kohonen::som(DIST, grid = somgrid(10, 10, "hexagonal", 
...
```{r}
plot.silhouette.score(SOM_nat, clust_method="complete", interval=seq(2,30))
```
...
add.cluster.boundaries(SOM_nat, SOM.hc, lwd=5)
...
Sx_file <- system.file("data/03_HIF_dataset/Sx.dat", package="SOMMD")
Sx <- read.table(Sx_file)
Neur.avg.Sx <- average.neur.property(SOM_nat, Sx[,1])

Zx_file <- system.file("data/03_HIF_dataset/Zx.dat", package="SOMMD")
...
```{r}
SOM_sel <- kohonen::som(DIST, grid = somgrid(10, 10, "hexagonal", 
                                         neighbourhood.fct="gaussian", toroidal=FALSE), 
                    dist.fcts="euclidean", rlen=1000, mode='pbatch')
```
...
```{r}
plot.silhouette.score(SOM_sel, clust_method="complete", interval=seq(2,30))
```
...
add.cluster.boundaries(SOM_sel, SOM.hc, lwd=5)
...
plot(SOM_sel, type = "mapping", bgcol=COL.SCALE[SOM.hc], col=rgb(0,0,0,0), 
       shape='straight', main="")
add.cluster.boundaries(SOM_sel, SOM.hc, lwd=3)
...
plot(net, edge.arrow.size=E(net)$width/5, edge.curved=0.17, edge.color='black', vertex.label="", layout=SOM_sel$grid$pts)
```
...
```{r}
coords = layout_with_fr(net, coords=SOM_sel$grid$pts)
...
par(mfrow=c(1,2))
#Do the plot for Sx
COL <- map2color(Neur.avg.Sx, col.palette)
plot(net, edge.arrow.size=E(net)$width/10, edge.curved=0.17, vertex.color=COL, edge.color='black', vertex.label="", layout=coords)

#Do the plot for Zx
COL <- map2color(Neur.avg.Zx, col.palette)
plot(net, edge.arrow.size=E(net)$width/10, edge.curved=0.17, vertex.color=COL, edge.color='black', vertex.label="", layout=coords)
```
