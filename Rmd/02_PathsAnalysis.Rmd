...
folder <- system.file("data/02_ARNT_dataset/", package="SOMMD")
xtcfiles <- list.files(folder, pattern = "*.xtc")
pdbfile <- system.file("data/02_ARNT_dataset/ref.pdb", package="SOMMD")

#Read the first replica
trj <- read.trj(trjfile=paste(folder, "/", xtcfiles[1], sep=''), topfile=pdbfile)
...
pdb <- bio3d::read.pdb("../data/02_ARNT_dataset/ref.pdb")
...
folder <- system.file("data/02_ARNT_lowspeed_dataset/", package="SOMMD")
xtcfiles <- list.files(folder, pattern = "*.xtc")

#Read the first replica
trj2 <- read.trj(trjfile=paste(folder, "/", xtcfiles[1], sep=''), topfile=pdbfile)
#Append all other trj files
for(trj_file in xtcfiles[-1]){
  rep <- read.trj(trjfile=paste(folder, "/", trj_file, sep=''),
                  topfile=pdbfile)
  trj2 <- cat.trj(trj, rep)
}
...
```{r}
#Map new data on the existing SOM
boxplot(SOM$distances, xlim=c(0.5,2.5), ylim=c(0,80), pch=16, ylab="Distance")
boxplot(SOM_new$distances, at=2, add=TRUE, pch=16)
axis(1, at=c(1,2), labels=c("High Speed", "Low Speed"))
```

As can be seen the distances from the SOM vectors in the two cases are similar. Slightly higher values were obtained for the data mapped on SOM, but this is expected as the SOM is optimized to represent the first set of data.
